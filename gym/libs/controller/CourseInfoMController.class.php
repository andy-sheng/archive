<?php 
	class CourseInfoMController
	{
		...
		function display($id)
		{
			...
			$this->smarty->display('libs/view/courseInfoM.ctp');	
		...
		function writeInfo($post)
		{
			...
			$db->setCourseInfoById($post['courseid'], $post);
		}
		function handle($g, $p, $s)
		{
			...
			if(isset($this->post['courseid'])){//带了表单数据
				$this->writeInfo($p);
				header('location:index.php?action=projmanag&operation=success');
				exit();
			}
			else{//不带表单数据
				$this->display($g['id']);
