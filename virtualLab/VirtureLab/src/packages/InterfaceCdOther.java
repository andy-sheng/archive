 ...
 * @author AndySheng
 */
public class InterfaceCdOther extends javax.swing.JFrame {
     ...
     */
    public InterfaceCdOther(String[] experimentInfo,PersonalInfo persInfo,InterfaceMain interfaceM,ClientSocket c) throws FileNotFoundException, IOException {//expInfo[0]=name expInfo[1]=entity
		personalInfo = persInfo;
		interfaceMain = interfaceM;
		client = c;
		expInfo = experimentInfo;
		vhds = new Vector();
		vhds.addElement("other");
    	try {
    ...
    private void submitMouseClicked(java.awt.event.MouseEvent evt) throws IOException {                                    
		...
		check.mkdirs();
		new SetPin(personalInfo.getNumber(),"other");
//		checkBox needSetBtn = new checkBox("是否要设置按钮？");
//		if(needSetBtn.getResult())
//			new SetBtn(this,true,personalInfo.getNumber());
		InputFenpin fenpin = new InputFenpin();
		String fenpinNu = "";
		if(fenpin.getFlag())
			fenpinNu = fenpin.getClassNumber();
		try{
			client.send("022");
			System.out.println("发送022");
		}catch (Exception e1) {}
		try {
			final String[] currentExp = new String[5];
			...
			currentExp[0] = expInfo[0];
			currentExp[1] = "";
			currentExp[2] = new Time().getTime();
			currentExp[3] = "";
			currentExp[4] = "";
			 ...
			 * 发送文件目录
			 */
			client.sendVector(vhds);
			client.send(fenpinNu);
