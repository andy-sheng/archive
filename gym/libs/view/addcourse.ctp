	...
	<div class='jumbotron'  style='max-width:700px;margin:auto'>
		...
		<form role="form" id='form' class='regForm' method="POST" action="">
		 	...
		 	<div class="form-group">
      		...
      		<div class="form-group">
       			<label id='exampleInputPassword1' for="inputError">选择教练</label>
...
{literal}
	...
	function confirm()
	{
		if($('#datetime').val() == ''){
			...
			if(period == '凌晨' || period == '早上' && hour < 9 || period == '晚上' && hour >= 9){
				...
				document.all("form").setAttribute("action", "index.php?action=projmanag&add=confirm");
				$('#form').submit();
			}
		}
	...
	function cancel()
	{
		document.all("form").setAttribute("action", "index.php?action=projmanag");
		$('#form').submit();	
	}
